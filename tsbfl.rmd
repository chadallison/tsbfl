...
```{r echo = F}
data.frame(x = c("Times Jerry has sent in his video", "Times I've thought about killing myself"), y = c(0, 73)) |>
  mutate(x = factor(x, levels = c("Times Jerry has sent in his video", "Times I've thought about killing myself"))) |>
  ggplot(aes(x, y)) +
  geom_col(fill = "#BCA8C8") +
  geom_text(aes(label = y), vjust = -0.5) +
  labs(x = NULL, y = NULL, title = "Very informative and important graph")
```
...
get_optimal_score = function(team, wk) {
  ...
  rb = df |> filter(pos == "rb" & used == 0) |> slice_max(pts, n = 1, with_ties = F) |> pull(pts)
  rbn = df |> filter(pos == "rb" & used == 0) |> slice_max(pts, n = 1, with_ties = F) |> pull(pnum)
  df = df |> mutate(used = ifelse(used == 1 | pnum == rbn, 1, 0))
  wrte = df |> filter(pos %in% c("wr", "te") & used == 0) |> slice_max(pts, n = 1, with_ties = F) |> pull(pts)
  wrten = df |> filter(pos %in% c("wr", "te") & used == 0) |> slice_max(pts, n = 1, with_ties = F) |> pull(pnum)
  df = df |> mutate(used = ifelse(used == 1 | pnum == wrten, 1, 0))
  ...
  x2 = df |> filter(pos %in% c("rb", "wr", "te") & used == 0) |> slice_max(pts, n = 1, with_ties = F) |> pull(pts)
  x2n = df |> filter(pos %in% c("rb", "wr", "te") & used == 0) |> slice_max(pts, n = 1, with_ties = F) |> pull(pnum)
  df = df |> mutate(used = ifelse(used == 1 | pnum == x2n, 1, 0))
  x3 = df |> filter(pos %in% c("rb", "wr", "te") & used == 0) |> slice_max(pts, n = 1, with_ties = F) |> pull(pts)
  x3n = df |> filter(pos %in% c("rb", "wr", "te") & used == 0) |> slice_max(pts, n = 1, with_ties = F) |> pull(pnum)
  df = df |> mutate(used = ifelse(used == 1 | pnum == x3n, 1, 0))
...
```{r include = F}
missed_records = data.frame(team = teams_vec, week = week_vec) |>
  rowwise() |>
  mutate(actual_pts = get_score(team, week),
         optimal_pts = get_optimal_score(team, week)) |>
  ungroup() |>
  ...
  summarise(actual_pts = sum(actual_pts),
            optimal_pts = sum(optimal_pts)) |>
  mutate(missed_pts = optimal_pts - actual_pts) |>
  select(team, missed_pts) |>
  inner_join(team_records |>
  select(team, pct), by = "team")

missed_records |>
  ggplot(aes(missed_pts, pct)) +
  # geom_point(aes(col = team), size = 5, show.legend = F) +
  ggrepel::geom_text_repel(aes(label = team), size = 3) +
  geom_vline(xintercept = mean(missed_records$missed_pts), linetype = "dashed", alpha = 0.5) +
  geom_hline(yintercept = mean(missed_records$pct), linetype = "dashed", alpha = 0.5) +
  labs(x = "Missed Points (Full Season)", y = "Win Percentage",
       title = "How is lineup setting affecting team performance?")
```
